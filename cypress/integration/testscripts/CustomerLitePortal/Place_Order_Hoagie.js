...
describe('Place_Order Happy Path - Hoagies Scenario', () => {
    ...
    var url, location, storename, date, time, hproductname, hvariety, cheesetype, varietyname, toppings, side, calorie, servecount, price;
    ...
    beforeEach(function () {
        cy.fixture('testData').then(function (data) {
            ...
            date = this.data.date
            time = this.data.time
            hproductname = this.data.hproductname
            hvariety = this.data.hvariety
            cheesetype = this.data.cheese
    ...
    it('Select the product from Product listing page - Hoagie scenario', function () {

        //Select the product
        productListingPage.getHoagiesProduct_link()
        cy.info("1. CLICK ON THE PRODUCTS LINK - HOAGIES ")

        //verify the prodcut details
        productListingPage.getHoagiesMenuText()

        //Add Submenus
        productListingPage.getHoagiesbundle()
        cy.info("4. CLICK ON THE HOAGIES BUNDLE TO ADD THE SUBMENUS")

        //Add variety
        productListingPage.getHoagiesVariety(hvariety, cheesetype)
        cy.info("5. SELECT AND ADD THE HOAGIE VARIETY - " + hvariety)
        cy.info("6. SELECT AND ADD THE HOAGIE CHEESE TYPE - " + cheesetype)

        //Add toppings
        productListingPage.getToppings(toppings)
        cy.info("7. SELECT AND ADD THE HOAGIE TOPPINGS - " + toppings)

        //Click on chicken topping
        productListingPage.getChickenNoodleSoup(side)
        cy.info("8. SELECT AND ADD THE SIDE ITEMS - " + side)
    })
    ...
    it('Verify the Product and Checkout from Cart Details page', function () {
        ...
        //cy.info("1. CLICK ON 'CHECKOUT BUTTON' " + productname)
        cy.info("1. SELECT AND ADD THE HOAGIE VARIETY - " + hvariety)
        cy.info("2. SELECT AND ADD THE HOAGIE CHEESE TYPE - " + cheesetype)

        cartDetailsPage.getCheckoutButton()
        cy.info("3. CLICK ON 'CHECKOUT BUTTON' ")
        cy.wait(10000)
