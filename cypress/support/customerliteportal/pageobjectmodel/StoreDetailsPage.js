class StoreDetailsPage {
    ...
    getDate(date) {
        ...
        //const dategiven = cy.get('button[class="react-calendar__tile react-calendar__tile--now react-calendar__tile--active react-calendar__tile--rangeStart react-calendar__tile--rangeEnd react-calendar__tile--rangeBothEnds react-calendar__month-view__days__day"]').next()
        dategiven.click()
    }
    ...
    verifyTimeNotSelected() {

        const startOrderbutton = cy.contains('Start Order')
        startOrderbutton.click()
        cy.get('div[class*="Select__Root"]').contains("This field is required").should('have.text', "This field is required")
    ...
    getSelectStoreButtonErrorMessage() {
        ...
        storebutton.get('div.error-label').then(function (element) {
            ...
            cy.log(actualText)
            cy.info("1. VERIFY THE ERROR MESSAGE" + actualText)
        })
    }
